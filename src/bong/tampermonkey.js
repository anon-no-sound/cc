...
(function () {
    ...
    const listBannedUsers = async (page) => {
        return fetch(`https://rf.bongacams35.com/blocked-users?page=${page}`, {
            ...
            .then((r) => {
            console.log(r);
            const usernames = r.data.usersList.items.map((i) => i.username);
    ...
    const getCSRFToken = () => {
        const csrfElement = document.querySelector(`[data-${csrfKey}]`);
        if (!csrfElement) {
    ...
    const banMany = async (usernames, delay = 500) => {
    ...
    const unbanMany = async (usernames, delay = 500) => {
        for (const username of usernames) {
    ...
    const addLinkButton = (toolbar, id, text, onClick) => {
        if (toolbar.querySelector(`#${id}`))
            return;
        const btn = GM_addElement(toolbar, "div", {
            id,
            class: "bc_inline_flex bc_flex_full_center mplg_btn bc_mrn_btn __hint plt_btn __light bc_inline_flex bc_flex_full_center",
        });
        btn.onclick = onClick;
